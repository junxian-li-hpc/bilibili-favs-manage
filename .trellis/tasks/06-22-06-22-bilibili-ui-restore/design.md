...
## 0. 关键约束与领域事实（2026-06-27 确认）

> 本节为用户在真机验证后确认的关键信息，是跨页面流程设计的根本依据。
> **修改跨页面逻辑前务必先读本节。**

### 0.1 B 站行为：他人收藏夹页面的复制对话框
- 在**他人收藏夹页面**点击「复制」后，会弹出**页内窗口（modal）**，其中显示的是**「我（当前登录账号）自己」拥有的收藏夹列表**，可作为复制目标选择。
- 但该对话框**不能在其中新建收藏夹**——这是脚本需求与 B 站逻辑的核心冲突点。

### 0.2 由此推导出的跨页面流程（必须保留，方案正确）
用户在脚本里勾选源收藏夹并填写目标名称，但目标名称可能不在"我自己"的收藏夹列表中。因此：
1. 在他人页面选好源 + 目标名称，点击开始；
2. 若存在"我"还没有的目标收藏夹 → 跳转到**自己的收藏夹页面**对比并创建缺失的夹（**创建只能在自己页面进行**）；
3. 创建完成后**跳回他人页面**，此时复制对话框已能选到这些目标夹 → 执行复制。

→ 这正是当前三阶段状态机（`creating` → `copying`）的存在理由。**方案正确，应继续完善而非推翻。**

### 0.3 硬约束：禁止直接调用 B 站 API
- **只允许纯 DOM 模拟用户点击**，禁止直接发起 B 站接口请求（如 `createFavoriteViaAPI` / `addVideosToFavorites` / BV→aid 转换等）。
- 原因：① 直接调 API 有**封号风险**；② 上一个 AI 的 API 重写**破坏了原有功能、偏离了本设计**，已被回退并暂存于 `git stash@{0}`。
